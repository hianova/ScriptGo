...
use script_go::binary::{deserialize_sgb, serialize_sgb, SGB_MAGIC};
...
pub fn run_cmd(file_path: &str) -> Result<VmResult, String> {
    ...
    let bytecode = if bytes.len() >= covopt_param!("M_202_37", 4) && bytes[0..covopt_param!("M_202_51", 4)] == SGB_MAGIC {
...
pub fn check_cmd(file_path: &str) -> Result<(), String> {
    ...
    if bytes.len() >= covopt_param!("M_229_22", 4) && bytes[0..covopt_param!("M_229_36", 4)] == SGB_MAGIC {
    ...
    writeln!(std::io::stdout(), "[CHECK] OK: {} (Syntax and types valid)", file_path).unwrap();
    Ok(())
}
...
pub fn watch_cmd(file_path: &str, max_iterations: Option<usize>) -> Result<ScriptVm, String> {
    ...
    writeln!(std::io::stdout(), "[WATCH] Initial execution finished for {}: {:?}", file_path, result).unwrap();
    ...
    loop {
        ...
        {
                last_mtime = mtime;
                writeln!(std::io::stdout(), "[WATCH] File modification detected in {}. Recompiling...", file_path).unwrap();

                match read_and_compile() {
                    Ok(bytecode) => { let bytecode: Vec<Instruction> = bytecode;
                        vm.hot_reload();
                        match vm.run(&bytecode) {
                            ...
                            Err(e) => {
                                writeln!(std::io::stdout(), "[WATCH] VM execution error post hot-reload: {:?}", e).unwrap();
                            }
                        }
                    }
                    Err(e) => {
...
pub fn build_asm(input_path: &str, output_path: &str) -> Result<(), String> {
    ...
    writeln!(std::io::stdout(), "Successfully built {} -> {}", input_path, output_path).unwrap();
    Ok(())
}
...
pub fn print_help() {
    ...
    writeln!(std::io::stdout(), "  sgo run <file.sgo>                Compile and execute SGL script on ScriptVm").unwrap();
    writeln!(std::io::stdout(), "  sgo check <file.sgo>              Parse and type-check SGL script without running").unwrap();
    writeln!(std::io::stdout(), "  sgo compile <file.sgo> [-o out]   Compile SGL script into binary bytecode (.sgb)").unwrap();
    writeln!(std::io::stdout(), "  sgo watch <file.sgo> [--once]     Watch SGL script, recompile & hot-reload on change").unwrap();
    writeln!(std::io::stdout(), "  sgo --replay <trace.json>         Replay VM execution trace").unwrap();
    writeln!(std::io::stdout(), "  sgo --build <in.asm> <out.sgb>    Assemble SGL assembly source into bytecode").unwrap();
}
